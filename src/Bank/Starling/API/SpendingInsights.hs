...
byCounterParty (AccountUid accountUid) year month
  = getWithAuth ("/api/v2/accounts" </> accountUid </> "spending-insights/counter-party?year=" ++ show year ++ "&month=" ++ show month)
...
byCountry (AccountUid accountUid) year month
  = getWithAuth ("/api/v2/accounts" </> accountUid </> "spending-insights/country?year=" ++ show year ++ "&month=" ++ show month)
...
byCategory (AccountUid accountUid) year month
  = getWithAuth ("/api/v2/accounts" </> accountUid </> "spending-insights/spending-category?year=" ++ show year ++ "&month=" ++ show month)
